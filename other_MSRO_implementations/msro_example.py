...
if __name__ == "__main__":
    ...
    nb_simulations = 100
    min_size = 6
    max_size = 6  # if max_size == min_size, only one matrix size will be generated per simulation
    incrementation = 4
    ...
    for size in range(min_size, max_size + 1, incrementation):
        ...
        for count in range(nb_simulations):
            ...
            comparison.append(msro1_order == msro2_order)
            if count % 100 == 0:
                print(f"msro1 order : {msro1_order}")
                print(f"msro2 order : {msro2_order}")
                print()

        results = np.sum(comparison)
